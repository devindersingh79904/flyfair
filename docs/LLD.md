...
## 9. Alias Resolution & Multilingual Support

The search service implements deterministic, offline alias resolution using static JSON arrays to enable multilingual searching (English, Chinese, Japanese, Hindi, Arabic, Korean, and Latin diacritics) without relying on runtime translation APIs.

Aliases are resolved in the following priority order:
1.  **Curated Aliases (`aliases.json`)**: Highest priority (e.g., `LON`, `Bali`).
2.  **Multilingual Aliases (`multilingual_aliases.json`)**: Native names and non-Latin scripts (e.g., `北京`, `दिल्ली`, `دبي`).
3.  **Generated Aliases (`airports.json` / `city_groups.json`)**: Automatic variations based on IATA, city names, and names.

Normalization Rules:
-   **Latin Text**: Converted to lowercase, trimmed, collapsed spaces, and accent-folded using `unidecode` (e.g., `São Paulo` -> `sao paulo`, `München` -> `munchen`).
-   **Non-Latin Text**: Trimmed and collapsed, but original scripts are preserved (e.g., `東京` remains `東京`, `دبي` remains `دبي`). No automatic transliteration is performed on CJK, Hindi, or Arabic scripts.

Non-Latin scripts rely purely on **Exact Match** and **Prefix Match** pathways against the `multilingual_aliases.json` store. Fuzzy matching is limited to Latin-based strings to prevent arbitrary transliteration penalties.
...
## 9.1 Google Translate Fallback Flow

When a user submits a query, the system first executes a local search. If the local search yields weak results and the query contains non-English/non-Latin scripts, the system falls back to Google Cloud Translation API:

```mermaid
graph TD
    A[User query] --> B[Local alias/index search]
    B --> C{Weak result & non-English query?}
    C -- No --> D[Return local response]
    C -- Yes --> E[Google Translate fallback]
    E --> F[English query search]
    F --> G{Translated top score > local top score?}
    G -- Yes --> H[Return translated results with metadata]
    G -- No --> D
```

### Flow Steps:
1. **Local Search**: Run `_search_local(query)`.
2. **Weak Check**: A result is considered "weak" if no results are found, or the top result score is less than `TRANSLATION_WEAK_RESULT_THRESHOLD` (default: `650`).
3. **Non-English Script Detection**: Verify if the query contains characters in non-Latin ranges (Thai, Han/CJK, Hiragana/Katakana, Hangul, Arabic, Cyrillic, Devanagari) and length >= `TRANSLATION_MIN_QUERY_LENGTH`.
4. **Translation**: If enabled, translate the query using Google Cloud Translation REST API, checking an in-memory LRU cache first.
5. **Secondary Local Search**: Search local indexes again with the translated query.
6. **Compare & Return**: If the secondary search has a higher top score than the primary search, return it with translation metadata (`translationFallbackUsed=True`, `translatedQuery`, `detectedLanguage`, `translationProvider`). Otherwise, return the primary local search results.

---

### 4.2 Data Storage

The static databases generated by the above flow are stored in `backend/app/data/`:
*   `airports.json`: Generated airport array.
*   `city_groups.json`: Generated city groups array.
*   `countries.json`: Generated country mapping array.
*   `regions.json`: Curated region mapping array.
*   `aliases.json`: Manually curated, top-priority aliases.
*   `multilingual_aliases.json`: Generated/Enriched offline aliases for multi-language and multi-script support (e.g., CJK, Arabic, Hindi).t[SearchableItem]
...
class AirportSearchService:
    ...
    def search(self, query: str, limit: int = 10) -> list[SearchResult]:
        ...
        results = suppress_group_child_airport_duplicates(results)
...
**Note on Duplicate Suppression:** After ranking and deduplication, top-level airport results that are already nested inside returned city/region groups are suppressed to avoid duplicate UI rows.
...
def _match_fuzzy(self, normalized_query: str, results: list[SearchResult]) -> None:
    for item in self.index.searchable_items:
        ...
        results.append(item.to_search_result(
            ...
            matched_value=normalized_query
        ))
...
If a query matches a country code, exact country name, or a country alias (e.g., "United Kingdom", "UK", "UAE"), the system will fall back to returning the top-ranked city groups and major commercial airports associated with that country.

```python
def _match_country(self, normalized_query: str, results: list[SearchResult]) -> None:
    # Match logic mapped in code returning MatchReason.COUNTRY_EXACT, 
    # MatchReason.COUNTRY_ALIAS or MatchReason.COUNTRY_PREFIX
    pass
```
...
The application supports containerized deployment via Docker Compose. 

- **Backend**: Uses a `python:3.12-slim` base image. It copies dependencies and the application folder and serves via `uvicorn` on port 8000. It reads from `.env` loaded via Docker Compose `env_file`.
- **Frontend**: Uses a multi-stage Docker build. 
  - **Build stage**: Uses `node:20-alpine` to compile the Vite React app. The `VITE_API_BASE_URL` is passed as a build argument. 
  - **Note on Browser Accessibility**: The API URL must be `http://localhost:8000/api/v1` (or the host's actual IP) rather than the internal Docker network name `backend`, because requests are initiated by the client's web browser on the host machine.
  - **Production stage**: Uses `nginx:alpine` to serve static assets on port 80.
- **Docker Compose**: Wires both frontend and backend together. Exposed ports are 8000 for the backend API and 5173 for the frontend NGINX server.
